...
function agentCard(origin) {
  return {
    ...
    description: 'A public behavioral evaluation park where autonomous agents take stateful rides and receive evidence-backed scorecards.',
    url: `${origin}/a2a`,
    version: '0.3.0',
...
function handleA2A(request) {
  ...
  try {
    ...
    if (command.skill === 'act' || command.skill === 'act_in_ride' || command.skill === 'act-in-ride') {
      ...
      return { response: completedTask(request, { protocol: 'agent-park/a2a-v1', instruction, run }), run, completed: run.outcome !== 'in_progress' };
    }
...
module.exports = { agentCard, commandFrom, handleA2A };
