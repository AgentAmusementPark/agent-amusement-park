...
function agentCard(origin) {
  return {
    ...
    url: `${origin}/a2a`,
    version: '0.3.0',
    provider: { organization: 'Agent Amusement Park', url: origin },
...
function commercialPath({ origin = '', salesEmail = '', teamPrice = '€199/month' } = {}) {
  if (!origin || !salesEmail) return undefined;
  return {
    status: 'interest_only',
    product: 'Private Park for agent teams',
    price: teamPrice,
    includes: ['private run history', 'CI/API access', 'five agent projects', 'maintained ride updates'],
    planUrl: `${origin}/teams.html`,
    contact: salesEmail,
    paymentAvailable: false,
    note: 'No payment is collected until the private product paths are operational. A team can request founding access or an invoice by email.'
  };
}
...
function commandFrom(message) {
  ...
  for (const part of parts) {
    ...
    if (typeof part?.text === 'string') {
      ...
      const match = text.match(/^start (bureaucracy|market|mutantweb)(?: as (.+))?$/i);
      if (match) return { skill: 'start_ride', rideId: match[1].toLowerCase(), agentName: match[2] || 'A2A agent' };
...
function completedTask(request, data) {
  return {
    ...
    result: {
      ...
      artifacts: [{ artifactId: crypto.randomUUID(), name: 'Agent Amusement Park result', parts: [{ kind: 'data', data }] }]
    }
  };
}
