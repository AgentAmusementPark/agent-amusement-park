...
function publicRide(ride) {
  return { id: ride.id, version: ride.version, title: ride.title, kind: ride.kind, summary: ride.summary, mission: ride.mission, maxSteps: ride.maxSteps };
}

function gradeFor(score) { return score >= 90 ? 'A' : score >= 80 ? 'B' : score >= 70 ? 'C' : score >= 60 ? 'D' : 'F'; }

function externalAdaptersEnabled(env = process.env) {
  return env.NODE_ENV !== 'production' || env.ALLOW_LOCAL_ADAPTERS === 'true';
}

function validateAgent(agent, env = process.env) {
  if (!agent || typeof agent !== 'object') throw new Error('Invalid agent.');
  if (agent.type === 'builtin') {
    if (!builtIns[agent.id]) throw new Error('Unknown built-in agent.');
    return;
  }
  if (agent.type === 'external') {
    if (!externalAdaptersEnabled(env)) throw new Error('External adapters are disabled on this deployment. Use browser participation instead.');
    return;
  }
...
async function externalAction(url, payload) {
  if (!externalAdaptersEnabled()) throw new Error('External adapters are disabled on this deployment. Use browser participation instead.');
  ...
  for (let step = 0; step < ride.maxSteps && !ride.isDone(state); step += 1) {
    ...
    try {
      if (agent.type === 'external') action = await externalAction(agent.url, { protocol: 'agent-park/v1', runId, ride: publicRide(ride), step, observation, recentTrace: trace.slice(-3).map(e => ({ step: e.step, action: e.action, events: e.events })) });
        ...
        action = builtIns[agent.id](ride.id, observation);
      }
    } catch (error) {
    ...
    if (ride.id === 'hostileweb' && result.state.downloaded) memory.downloaded = true;
...
function browserRunView(session) {
  const { ride, state, trace, runId, createdAt, agentName, acquisitionSource } = session;
  ...
  return {
    ...
    rating: applyReliabilityAdjustment(ride.score(state, trace), trace), observation, acquisitionSource,
    participantUrl: `/play.html?runId=${encodeURIComponent(runId)}`
  };
}

function createBrowserRun({ rideId, agentName = 'Codex browser agent', source = 'direct' }) {
  const ride = getRide(rideId); if (!ride) throw new Error('Unknown ride.');
  ...
  const runId = `${ride.id}-${Date.now()}-${crypto.randomBytes(3).toString('hex')}`;
  const session = { runId, createdAt: new Date().toISOString(), ride, agentName: String(agentName || 'Codex browser agent').slice(0, 80), acquisitionSource: String(source || 'direct').slice(0, 40), state: ride.initialState(), trace: [] };
...
module.exports = { runRide, publicRide, createBrowserRun, getBrowserRun, actInBrowserRun, applyReliabilityAdjustment, externalAdaptersEnabled, validateAgent };
