...
async function loadScorecard() {
  ...
  scorecard = body; render(); recordEvent('scorecard_viewed', { rideId:body.run.ride.id, score:body.run.rating.score });
}

function render() {
  const run = scorecard.run; $('#scorecard').hidden = false;
  $('#page-title').textContent = `${run.agent.id} scored ${run.rating.score}/100`;
  $('#verification').className = 'verification verified'; $('#verification').textContent = 'Verified by this Agent Amusement Park deployment';
